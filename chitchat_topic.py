...
try:
    ...
    st.error(f"Error creating or loading embeddings and FAISS index: {e}")
    raise
...
def retrieve_documents(query, topics=None, top_k=3):
    try:
        ...
        st.error(f"Error retrieving documents because the model cannon understand your request: {e}. Try another query!")
...
def summarize_document(title, content):
    try:
        ...
        st.error(f"Error summarizing document '{title}': {e}")
...
def topic_classifier(query):
    try:
        ...
        st.error(f"Error classifying topic for query '{query}': {e}. You can Try Again!")
        return ["Other"]
    
####################################################
# Function to visualize topic counts in chat history
def visualize_topic_counts(chat_history, enable_auto_topic_classification):
    topics = []
    for message in chat_history:
        if message["role"] == "user" and enable_auto_topic_classification:
            classified_topics = topic_classifier(message["content"])
            topics.extend(classified_topics)
            st.write(f"Classified topics for message '{message['content']}': {classified_topics}")
    if not topics:
        st.write("No topics found in the chat history.")
        return
...
def chat_with_model(user_input, max_tokens=256):
    try:
        ...
        st.error(f"Error during chat with model, the model cannon understand your request: {e}. Please try to input again.")
...
if st.sidebar.button("Visualize Topic Counts"):
    st.write("Visualizing topic counts...")
    visualize_topic_counts(st.session_state.chat_history, enable_auto_topic_classification)

####################################################
# Topic selection
topic_options = list(topic_documents.keys())
topic_options.insert(0, "None")
selected_topics = st.sidebar.multiselect("Choose topics for document retrieval", topic_options, default=["None"])
selected_topics = [topic for topic in selected_topics if topic != "None"]
selected_topics = None if not selected_topics else selected_topics
####################################################
